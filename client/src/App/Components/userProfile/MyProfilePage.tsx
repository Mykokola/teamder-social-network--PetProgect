...
//likeStatus ? await plusLike() : await minusLike()
 await getUser({_id:id})
  ...
  console.log(likeStatus)
   ...
   if(data){
    const {avatarURL,name,surName,login,bio,interess,city,age,likes} = data.user[0]
    return (
      <ProfileContainer>
        <HeaderContainer>
    ...
    <UserInteresContainer>
      ...
      <UserInteresParagraf>{interess.map((e:string,i:number) =>  <div key={i}>{e} </div>)}</UserInteresParagraf>
    </UserInteresContainer> 
    <UserInfoList>
      <UserInfoItem>
        <svg
